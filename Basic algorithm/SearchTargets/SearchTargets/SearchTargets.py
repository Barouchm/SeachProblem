...
big_plot = False
Areas_colors=[(220,220,220),(169,169,169),(119,136,153),(200,200,220),(150,150,150),(119,120,170),(200,135,235),(140,140,140),(100,180,130),(170,220,200),(1,140,170),(1,135,235),(1,140,140),(1,180,130),(1,220,200)]
...
a=[Agent(0,0) for i in range(NumberOfAgents)]
#a=[Agent(11,16),Agent(0,14),Agent(7,1)]#,Agent(4,4),Agent(10,10)]#,Agent(10,7),Agent(4,4),Agent(10,10)]
#a=[Agent(25,3),Agent(20,9),Agent(22,35)]
#a=[Agent(9,4)]#,Agent(5,5)]
...
#Targets=[Target(5,0),Target(0,9)]#,Target(7,1)]
...
#False Alarm
Estimated_FA=[2]; #Estimated False alarm rate per sensor

sum_max_avg_reward=0

FalseAlarmParameterGenerator=[2];
    ...
    for j in range(N):
        for m in range(NumberOfAgents):
            for k in range(len(s)):
                P[i][j][m][k][0]=NumberOfTargets/(N*N)
...
def GeneratePlots(t,isSharing):
    ...
    if(big_plot == False):
        ...
        if(isSharing==True):
            ...
            plt.title("Global probability map   t="+ str(t))
        else:
        ...
        plt.title("Agents-Green, Targets-Red ")
        for tar in range(NumberOfTargets):
            targets_agents_pos[Targets[tar].x_pos][Targets[tar].y_pos]=(255,255,255) # reset red
    ...
    plt.xticks(range(NumberOfTargets),["("+str(Targets[tar].x_pos)+","+str(Targets[tar].y_pos)+")" for tar in range(NumberOfTargets)],fontsize=6,rotation=300)
    for tar in range(NumberOfTargets):
        if(time_arr[tar]!=0):
             #plt.text(tar,time_arr[tar],"t="+str(time_arr[tar]),fontsize=8)
             plt.text(tar,time_arr[tar],str(time_arr[tar]),fontsize=8,ha='center')
...
def dynamic_targets_movement():
    for tar in range(NumberOfTargets):
        ...
        if(rand<pr_static/100):
            ...
            if(Targets[tar].x_pos+dir_x<0 or Targets[tar].x_pos+dir_x>=N):
                continue
            if(Targets[tar].y_pos+dir_y<0 or Targets[tar].y_pos+dir_y>=N):
                continue
            Targets[tar].x_pos+=dir_x
            Targets[tar].y_pos+=dir_y
...
def Sensor_Probability(t):
    ...
    temp_mat=[[[[0 for m in range(NumberOfAgents)]for k in range(len(s))] for _ in range(N)] for _ in range(N)]
    for i in range(N):
        for j in range(N):
                ...
                for m in range(NumberOfAgents):
                    ...
                    for ind in [(i-1,j-1),(i-1,j),(i-1,j+1),(i,j-1),(i,j+1),(i+1,j-1),(i+1,j),(i+1,j+1)]:
                        if(ind[0]>=0 and ind[0]<=N-1 and ind[1]>=0 and ind[1]<=N-1):
                            count+=1
                            directions_sum+=P[ind[0]][ind[1]][m][k][t-1]
                    directions_sum+=(8-count)*P[i][j][m][k][t-1]
                    temp_mat[i][j][k][m]=P[i][j][m][k][t-1]*pr_static/100+directions_sum*(1-pr_static/100)/8
    ...
    for i in range(N):
        ...
        #for every every cell:
            for k in range(len(s)):
                for m in range(NumberOfAgents):
                    P[i][j][m][k][t-1]=temp_mat[i][j][k][m]
    ...
    for i in range(N):
        for j in range(N):
            for k in range(len(s)):
                ...
                p_s_x0=P[i][j][m][k][t]*(1-1.0/Estimated_TE[k]*s[k].A*exp(-R_new/s[k].pow))/((1-P[i][j][m][k][t]*1.0/Estimated_TE[k]*s[k].A*exp(-R_new/s[k].pow))+(1-P[i][j][m][k][t])*(1-1.0/Estimated_FA[k]*s[k].A*exp(-R_new/s[k].pow)))
               
                p_s_x1=max(p_s_x1,10**(-100))
                p_s_x0=max(p_s_x0,10**(-100))
                P[i][j][m][k][t]=max(P[i][j][m][k][t],10**(-100))
                #if (P[i][j][m][k][t]>0.000000000001 and P[i][j][m][k][t]<1.0):
...
def calculate_EIG_global2Voronoi(t,agent_location_x,agent_location_y,new_agent_location_x,new_agent_location_y, voronoiArea):
    #P_cell[t]
    if(new_agent_location_x<0 or new_agent_location_x>=N):
        return -100000
    if(new_agent_location_y<0 or new_agent_location_y>=N):
...
def move_decision_EIG_global(t):
    ...
    for m in range(NumberOfAgents):
        ...
        #print(a[m].x_pos)
        #print(max_new_agent_location_x)
        if max_EIG>0.00000000000001:
           a[m].x_pos=max_new_agent_location_x
           a[m].y_pos=max_new_agent_location_y


def move_decision_EIG_global_Voronoi(t):
    Areas=[[] for _ in range(NumberOfAgents)]
    for i in range(N):
        for j in range(N):
            min_agent=0
            min_distance_squared=999999
            for m in range(NumberOfAgents):
                if(((a[m].x_pos-i)**2+(a[m].y_pos-j)**2)<min_distance_squared):
                    min_distance_squared = (a[m].x_pos-i)**2+(a[m].y_pos-j)**2
                    min_agent=m
            Areas[min_agent].append((i,j))
    ...
    for m in range(NumberOfAgents):
        ...
        for x_offset in [-1,0,1]:
            for y_offset in [-1,0,1]:
                ...
                new_agent_location_y=a[m].y_pos+y_offset
                #print(max_new_agent_location_x)
                if(calculate_EIG_global2Voronoi(t,a[m].x_pos,a[m].y_pos,new_agent_location_x,new_agent_location_y,Areas[m])>max_EIG):
                    max_EIG=calculate_EIG_global2Voronoi(t,a[m].x_pos,a[m].y_pos,new_agent_location_x,new_agent_location_y,Areas[m])
                    max_new_agent_location_x=new_agent_location_x
                    max_new_agent_location_y=new_agent_location_y
                    #print(" ")
                    #print(a[m].x_pos)
        ...
        #print(a[m].x_pos)
        #print(max_new_agent_location_x)
        if max_EIG>0.0000000000000000000000000000000000000000000001:
           a[m].x_pos=max_new_agent_location_x
           a[m].y_pos=max_new_agent_location_y
    return Areas
...
def Algorithm18_RT_EIG_Global_Voronoi(t):
    Rev_Func(t) #False alarm and true events generators + recieve event function 
    Sensor_Probability(t) #calculating the probability of a target for each sensor
    #Update_Estimated_FA(t)
    Agent_Probability(t) #sensors' probabilities integration
    ...
    Areas= move_decision_EIG_global_Voronoi(t)
    Calculate_reward(t)
    #sensor_power_modification(t,0.5,0.8,0.2)
    ...
    for m in range(NumberOfAgents):
        for (i,j) in Areas[m]:
            cell_color=targets_agents_pos[i][j]
            if(cell_color==(255,255,255) or cell_color in Areas_colors):
                targets_agents_pos[i][j] = Areas_colors[m]
    '''
    
    for i in range(N):
        for j in range(N):
            targets_agents_pos[i][j] =(255,255,255)
...
for t in range(1,T):
   ...
   # print("t="+str(t))
    ...
    Algorithm18_RT_EIG_Global_Voronoi(t)
